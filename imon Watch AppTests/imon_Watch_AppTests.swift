...
struct EvolutionEngineTests {
    @Test("Dotkin evolves to Hopkin after 1 hour")
    func dotkinToHopkin() {
        let start = Date.now
        var state = makeTestState(species: .dotkin, at: start)
    ...
    @Test("Hopkin evolves to Emberkin with 0-1 care mistakes")
    func hopkinToEmberkin() {
        let start = Date.now
        var state = makeTestState(species: .hopkin, at: start)
        state.evolvedAt = start
    ...
    @Test("Hopkin evolves to Marshkin with 2+ care mistakes")
    func hopkinToMarshkin() {
        let start = Date.now
        var state = makeTestState(species: .hopkin, at: start)
        state.evolvedAt = start
...
struct BattleEngineTests {
    ...
    func battleResult() {
        let state = makeTestState(species: .rexkin, strength: 4, weight: 30)
